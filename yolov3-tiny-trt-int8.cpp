...
string calibFile = "F:/TensorRT-6.0.1.5/data/v3tiny/calibaration.txt";

vector<string> labels = { "abnormal" };
const int Classes = 13;
vector<vector<int> > output_shape = { { 1, 15 + 3 * Classes , 13, 13 },{ 1, 15 + 3 * Classes, 26, 26 } };
...
vector<DetectionRes> postProcess(cv::Mat& image, float * output) {
	...
	for (int i = 0; i < output_shape.size(); i++) {
		auto shape = output_shape[i];
		vector<int> tmp = { shape[2], shape[3], 3, 5 + Classes };
			...
			for (int w = 0; w < shape[1]; w++) {
				...
				for (int c = 0; c < shape[2]; c++) {
					...
					for (int i = 5; i < 5 + Classes; i++) {
						score = max(score, sigmoid(ptr[4]) * sigmoid(ptr[i]));
					}
...
// 加载TensorRT引擎文件
bool readTrtFile(const std::string& engineFile, //引擎文件的名字
	IHostMemory*& trtModelStream)  //TensorRT模型的输出Buffer
{
	fstream file;
...
void doInferenceFrieza(IHostMemory* trtModelStream)
{
	...
	*/
	...
	std::cout << "Inference take: " << total / 20.0 << " ms." << endl;
